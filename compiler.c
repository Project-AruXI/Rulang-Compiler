...
#include "./common/pcre2.h" // There is a system pcre2 but it is older, use recent
#include "config.h"
...
static int compile(const char* filename) {
  rlog("Compiling Rulang file %s to assembly.", filename);
  return 1;
}
...
int main(int argc, const char** argv) {
  ...
  for (int i = 0, j = 0; infiles[i]; i++) {
    ...
    if (strcmp(ext, ".s") == 0 || strcmp(ext, ".asm") == 0 || strcmp(ext, ".as") == 0 || strcmp(ext, ".ars") == 0) {
      ...
      rlog("Compiling Rulang file %s", infiles[i]);
